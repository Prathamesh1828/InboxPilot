...
def _set_auth_cookie(response: Response, token: str) -> None:
    """
    Sets the session cookie with correct cross-site settings for
    Vercel (frontend) <-> Render (backend) production deployment.

    SameSite=None + Secure=True is required for cross-site cookie delivery.
    SameSite=Lax (the default) blocks cookies on cross-site POST requests,
    which is why login was silently failing in production.
    """
    response.set_cookie(
        key="session",
        value=token,
